# 🧠 DSA in Python — 10-Week Public Learning Journey

> Learning in public. Every problem, every concept, 
> every "aha" moment — documented here.

---

## 📌 About This Repo

10 weeks · ~10 hrs/week · 5 capstone projects  
Language: Python 3.x  
Platform: LeetCode

---

## 🗺️ Roadmap

| Phase | Weeks | Focus | Status |
|-------|-------|-------|--------|
| 🟣 Foundations | 1–2 | Big-O, Arrays, Patterns | ✅ Done |
| 🔵 Core Structures | 3–4 | Stacks, Linked Lists | ⏳ |
| 🟢 Trees & Search | 5–6 | BST, Sorting, Heaps | ⏳ |
| 🟡 Graphs | 7 | BFS, DFS, Topo Sort | ⏳ |
| 🟠 DP | 8–9 | Memoization, Tries | ⏳ |
| 🔴 Mastery | 10 | 2D DP, Backtracking | ⏳ |

---

## 📅 Weekly Logs

| Week | Topic | Daily Log | Blog | Status |
|------|-------|-----------|------|--------|
| [Week 1](./week-01/) | Python Foundations & Big-O | [📓 Log](./week-01/daily-log.md) | [Post](YOUR_BLOG_LINK) | ✅ |
| [Week 2](./week-02/) | Arrays & Patterns | [📓 Log](./week-02/daily-log.md) | [Post](YOUR_BLOG_LINK) | ✅ |
| Week 3 | Stacks & Queues | — | — | ⏳ |
| Week 4 | Linked Lists | — | — | ⏳ |
| Week 5 | Trees & Binary Search | — | — | ⏳ |
| Week 6 | Sorting & Heaps | — | — | ⏳ |
| Week 7 | Graphs | — | — | ⏳ |
| Week 8 | Dynamic Programming | — | — | ⏳ |
| Week 9 | Advanced Trees | — | — | ⏳ |
| Week 10 | Mastery & Review | — | — | ⏳ |

---

## 📊 Progress
Total Problems Solved :  29+ / 60+
Weeks Completed       :  2   / 10
Current Streak        :  14  day(s)
Hours Logged          :  35 hours

---

## 🔗 Follow Along

| Platform | Link |
|----------|------|
| 🐦 Twitter/X | #100DaysOfDSA |
| 📝 Blog | [YOUR BLOG LINK] |
| 💻 GitHub | This repo |

---

## 💡 Resources

- [NeetCode.io](https://neetcode.io)
- [LeetCode](https://leetcode.com)
- [Visualgo](https://visualgo.net)

---

*Started:1 March 2026 · Target: 30 June 2026*














